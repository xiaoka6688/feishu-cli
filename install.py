...
用法:
  ...
  python install.py --skip-auth              # 跳过 OAuth（之后手动跑 python auth_all.py）
"""
import argparse
import os
import platform
...
def main():
    ...
    p.add_argument("--skip-auth", action="store_true", help="跳过 OAuth（之后手动跑 python auth_all.py）")
    ...
    if not args.skip_auth:
        ...
        print("\n  跳过 OAuth。稍后跑: python auth_all.py")
    verify()
    ...
    print("  2. 同步 Wiki 树:      python sync_feishu_to_obsidian.py --wiki <WIKI_TOKEN>")
    print("  3. 同步单文档:        python sync_feishu_to_obsidian.py --doc <DOC_TOKEN>")
